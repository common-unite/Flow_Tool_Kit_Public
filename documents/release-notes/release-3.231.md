...
**Button visibility.** The **Email me a resume link** button is gated on a viable recipient being resolvable. It shows when Stages Mode + Allow Save Progress are enabled AND the submission has been saved AND at least one of: (a) the running user is logged in (not a guest), since internal or Experience Cloud portal users always have a viable `User.Email`; or (b) `Contact1_Email__c`, `Email__c`, or `Contact_1__c` is populated on the submission. For guests without any contact fields populated, the button is hidden to avoid sending a resume email to the Site Guest User's noreply address.
...
**New Custom Labels.** `Stages_Email_Me_Resume_Link`, `Stages_Draft_Saved`, `Stages_Resume_Link_Sent`, `Stages_Resume_Link_Error`, all overridable for branding or Translation Workbench localization.
...
**When it's suppressed.** Form Builder preview pane, Form Builder editor, repeater rows, sub-form rendering inside a parent form, and table-edit mode are all skipped by design.
