...
| `prefillRecord` | Output | SObject `Form_Submission__c` | Optional | Merged field-by-field over the local Form Submission: fields you set win, fields you leave out keep their existing value. Missing or null is a silent bypass; the form renders with whatever was already prefilled. Setting `Id` means the row already exists (your Flow inserted or found it): the form updates that record on every save instead of inserting a new one, which is how the resume path returns a saved draft. |
